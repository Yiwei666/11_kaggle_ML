...
# Increase the dataset size for [0, 4π]
x = np.linspace(0, 4 * np.pi, 600)  # Increased from 300 to 600
y = np.sin(x)

# Define a neural network model (same as before)
def build_model():
    model = Sequential()
    model.add(Dense(64, input_dim=1, activation='relu'))  # Input layer with 64 neurons
    model.add(Dense(128, activation='relu'))  # First hidden layer with 128 neurons
...
### 2. [0,4π] 500点7个隐藏层2500周期_256神经元数
...
### 1. [0,4π] 500点5个隐藏层2000周期_100神经元数
...
### 2. [0,4π] 500点5个隐藏层2000周期_120神经元数
